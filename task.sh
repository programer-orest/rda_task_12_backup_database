...
if [ -n "$DB_USER" ] && [ -n "$DB_PASSWORD" ]; then
  ...
  mysql -u $DB_USER_NAME -p$DB_USER_PASSWORD  ShopDBReserve < backup-no-create-db-no-create-info.sql
  mysql -u $DB_USER_NAME -p$DB_USER_PASSWORD  ShopDBDevelopment < backup-no-create-db-no-create-info.sql
else
